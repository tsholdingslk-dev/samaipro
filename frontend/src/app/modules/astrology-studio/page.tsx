                      ...
                      {(() => {
                        ...
                        return gridLayout.map((cell, idx) => {
                          if (cell.isCenter) {
                            if (idx === 5) {
                              return (
                                <div key={idx} style={{ gridColumn: "span 2", gridRow: "span 2", border: "1px solid rgba(139,92,246,0.15)", borderRadius: "8px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "rgba(139,92,246,0.03)", padding: "10px", textAlign: "center" }}>
                                  <div style={{ fontSize: "14px", fontWeight: 800, color: "#a78bfa" }}>
                                    {chartResult.lagna_sinhala} ලග්නය
                                  </div>
                                  <div style={{ fontSize: "11px", color: "#9ca3af", marginTop: "2px" }}>
                                    {chartResult.lagna} / {chartResult.lagna_tamil}
                                  </div>
                                </div>
                              );
                            }
                          ...
                          return (
                            <div 
                              key={r.id} 
                              style={{ 
                                border: "1px solid rgba(255,255,255,0.12)", borderRadius: "8px", padding: "6px",
                                background: r.id === lagnaPlanet.rasi_id ? "rgba(16,185,129,0.12)" : "rgba(255,255,255,0.02)",
                                display: "flex", flexDirection: "column", justifyContent: "space-between"
                              }}
                            >
                              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                <span style={{ fontSize: "0.75rem", fontWeight: 800, color: r.id === lagnaPlanet.rasi_id ? "#34d399" : "#9ca3af" }}>
                                ...
                                {planets.map((p: any) => {
                                  ...
                                  return (
                                    <span 
                                      ...
                                      style={{ fontSize: "0.72rem", background: "rgba(255,255,255,0.08)", color: lbl.color, padding: "1px 4px", borderRadius: "3px", fontWeight: 700, cursor: "pointer" }}
                                      title={p.name}
                                    >
                                      {getRichPlanetSymbol(p)}
                                    </span>
                              ...
                              <div style={{ display: "flex", flexWrap: "wrap", gap: "2px" }}>
                                {arudhas.map((a: string) => (
                                  <span key={a} style={{ fontSize: "8px", background: a === 'AL' ? '#fef08a' : (a === 'GL' || a === 'HL' || a === 'VL' || a === 'UL' ? '#dcfce7' : 'rgba(255,255,255,0.08)'), color: a === 'AL' ? '#854d0e' : (a === 'GL' || a === 'HL' || a === 'VL' || a === 'UL' ? '#166534' : '#9ca3af'), padding: "0px 3px", borderRadius: "2px", fontWeight: 700 }}>
                                    {a}
                                  </span>
                                ))}
                              </div>
