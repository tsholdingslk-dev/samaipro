...
import { 
  FlaskConical, Globe, Server, ExternalLink, RefreshCw, 
  Activity, CheckCircle2, AlertTriangle, ArrowLeft, Terminal,
  Layers, Shield, FileText, Users, Database, Sparkles
} from "lucide-react";

export default function LabNovaPortal() {
  const [serviceUrl, setServiceUrl] = useState<string>(() => {
    if (typeof window !== "undefined") {
      return localStorage.getItem("samai_labnova_url") || process.env.NEXT_PUBLIC_LABNOVA_URL || "http://localhost:3002";
    }
  ...
  const handleApplyUrl = (url: string) => {
    ...
    if (formatted) {
      ...
      if (typeof window !== "undefined") {
        localStorage.setItem("samai_labnova_url", formatted);
      }
      setIframeKey(k => k + 1);
    }
  };
  ...
  return (
    <div style={{ width: "100%", minHeight: "100vh", display: "flex", flexDirection: "column", background: "#0a0b10", color: "#f3f4f6", fontFamily: "'Inter', sans-serif" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
          <div style={{ display: "flex", alignItems: "center", background: "rgba(0,0,0,0.4)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "8px", padding: "2px 8px" }}>
            ...
            <input 
              ...
              placeholder="http://localhost:3002 or https://your-labnova.vercel.app"
          ...
          <div style={{ display: "flex", gap: "4px" }}>
            <button 
              onClick={() => { handleApplyUrl("http://localhost:3002"); }}
              ...
              title="Set to Local Microservice (Port 3002)"
            >
              Local (3002)
            </button>
            <button 
      ...
      <div style={{ flex: 1, position: "relative", display: "flex", flexDirection: "column", background: "#050608" }}>
        ...
        {/* If user selected Embedded or URL is live */}
          ...
          {isLive === false && serviceUrl.includes("localhost") && (
            <div style={{
              ...
              <div style={{ maxWidth: "680px", width: "100%", textAlign: "center" }}>
                <div style={{ width: "64px", height: "64px", borderRadius: "18px", background: "linear-gradient(135deg, rgba(16,185,129,0.2), rgba(5,150,105,0.1))", border: "1px solid rgba(16,185,129,0.3)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 1.5rem" }}>
                  <FlaskConical size={32} color="#10b981" />
                </div>

                <h2 style={{ fontSize: "1.8rem", fontWeight: 800, marginBottom: "0.8rem", color: "#fff" }}>
                  LabNova Microservice Standby
                </h2>
                <p style={{ color: "#9ca3af", fontSize: "0.95rem", lineHeight: 1.6, marginBottom: "2rem" }}>
                  LabNova Enterprise Portal runs as an independent Next.js microservice. You can run it locally on your computer or connect to a live cloud deployment.
                </p>
                ...
                <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "12px", padding: "1.2rem", textAlign: "left", marginBottom: "1rem" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "8px", fontWeight: 600, color: "#10b981", fontSize: "0.9rem", marginBottom: "0.5rem" }}>
                    <Terminal size={16} /> Option 1: Run Local Microservice (Port 3002)
                  </div>
                  <div style={{ background: "#000", padding: "0.8rem 1rem", borderRadius: "8px", fontFamily: "monospace", fontSize: "0.82rem", color: "#e5e7eb", overflowX: "auto" }}>
                    cd super_app_projects/labnova && npm run dev -- -p 3002
                  </div>
                </div>
                ...
                <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "12px", padding: "1.2rem", textAlign: "left", marginBottom: "2rem" }}>
                  ...
                  <p style={{ fontSize: "0.82rem", color: "#9ca3af", marginBottom: "0.8rem" }}>
                    Deploy <code style={{ color: "#818cf8" }}>super_app_projects/labnova</code> to Vercel/Railway and paste the URL in the bar above.
                  </p>
                  <div style={{ display: "flex", gap: "8px" }}>
                    <button 
                      onClick={() => {
                        const url = prompt("Enter your deployed LabNova URL:", "https://");
                        if (url) handleApplyUrl(url);
                      }}
                      style={{ background: "#6366f1", color: "#fff", border: "none", padding: "8px 16px", borderRadius: "8px", fontSize: "0.85rem", fontWeight: 600, cursor: "pointer" }}
                    >
                      Set Live Cloud URL
                    </button>
                    <button 
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "0.8rem", textAlign: "left" }}>
                  {[
                    { icon: Users, title: "Patient Registry", desc: "Diagnostic record tracking" },
                    { icon: FileText, title: "Lab Reports", desc: "Automated PDF generation" },
                    { icon: Shield, title: "Pathology CRM", desc: "Sample & test management" },
                  ].map((feat, i) => (
                    <div key={i} style={{ background: "rgba(0,0,0,0.3)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: "10px", padding: "0.8rem" }}>
                      <feat.icon size={16} color="#10b981" style={{ marginBottom: "0.3rem" }} />
